...
export default class Test extends React.Component<any, any> {
    ...
    onChange(info) {
        ...
        currFileList = currFileList.filter((file) => {
            //根据多选选项更新添加内容
            let hasNoExistCurrFileInUploadedList = !~findIndex(this.state.responseList, item => item['name'] === file.name);
            if (hasNoExistCurrFileInUploadedList) {
                if (!!this.props.isMultiple == true) {
                    this.state.responseList.push(file);
                } else {
                    // this.state['responseList'] = [file];
                }
            }
